...
# 2 nodes, each having 2 jobs sent in sequence
#
# use:
# ./<thisfile>.sh
...
#echo $inFolder  
#
echo "descrambling $prefix_fl ... "
sbatch zzz_sbatch_2nodes_1plus1jobs_rawraw_2_e_1k_4x250_1File.sh $inFolder $prefix_fl $suffix_fl0 $suffix_fl1 $outFolder $Img000to249 $Img250to499 $ADCcor $MultiGnCal $altPed $CMAcols $CDSFlag
sbatch zzz_sbatch_2nodes_1plus1jobs_rawraw_2_e_1k_4x250_1File.sh $inFolder $prefix_fl $suffix_fl0 $suffix_fl1 $outFolder $Img500to749 $Img750to999 $ADCcor $MultiGnCal $altPed $CMAcols $CDSFlag
echo " "
echo "all jobs submitted"
echo "  check status as:   squeue -u yourname"
echo "  then check for output files in $outFolder"
echo "--  --  --  --"
#
# to check:
# squeue -u marras

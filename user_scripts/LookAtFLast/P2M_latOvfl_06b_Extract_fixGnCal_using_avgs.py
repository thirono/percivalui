...
also e/ADU in fixGn(n)
=> calculate e/ADU of fixGn(n+1)
...
#
dflt_multiGnCal_file_Gnknown= "/gpfs/cfel/fsds/labs/percival/2020/calibration/20200505_000_BSI04_LatOvflw_PGA4or6BB/processed/fixGn_BSI04_Tm20_7of7_bSI04_05_PGA6BB/ramps/CalibrParam_fixGn/"+"BSI04_Tm20_7of7ADC_biasBSI04.05_fixGn0_PGA6_2020.06.06_fixGnCal_from3of7gapsAvg.h5"
#
...
#
dflt_fit_maxADU=0.75   # 75% of max #0.9 #up to 90% of max
dflt_fit_minR2=0.9
dflt_fit_minNpoints=4 #10 #4pt for having 2ke v.high-> med gn pixels
dflt_fit_slope=0.0 # slope non-negative
#
dflt_saveFile=dflt_multiGnCal_file_Gnknown+ '_to_fixGn1_PGAB.h5'
dflt_saveFile= 'NONE'
#
dflt_highMemFlag='Y'
dflt_cleanMemFlag= 'Y'
...
#
dflt_multiGnCal_file_Gnknown= "/gpfs/cfel/fsds/labs/percival/2020/calibration/20200505_000_BSI04_LatOvflw_PGA4or6BB/processed/fixGn_BSI04_Tm20_7of7_bSI04_05_PGA6BB/ramps/CalibrParam_fixGn/"+"BSI04_Tm20_7of7ADC_biasBSI04.05_fixGn1_PGAB_2020.06.06_fixGnCal.h5"
#
...
#
dflt_saveFile=dflt_multiGnCal_file_Gnknown+ '_to_fixGn2_PGAB.h5'
#dflt_saveFile= 'NONE'
#
dflt_highMemFlag='Y'
dflt_cleanMemFlag= 'Y'
...
if interactiveGUIFlag:
    ...
    #
    GUIwin_arguments+= ['save estimated (unknown-Gn) multiGnCal (PedestalADU, e/ADU) to file [NONE not to]']
...
if True:
    ...
    #
    if saveFlag: APy3_GENfuns.printcol('will unknown-Gn parameters to '+saveFile,'blue')
    #
...
#
#---
map_Gnknown_R2= APy3_GENfuns.numpy_NaNs((NRow,NCol))
map_Gnunkwn_R2= APy3_GENfuns.numpy_NaNs((NRow,NCol))
...
if interactiveShowFlag:
    ...
    while (nextstep not in ['q','Q']):
        ...
        if (nextstep in ['r','R']):
            ...
            e_per_ADU_Gnunkwn= numpy.NaN
            #
            map2fit= dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol]<=(fit_maxADU)*numpy.nanmax(dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol])
            data2fit_Gnknown_tint= numpy.copy(intTimes_ms_Gnknown[map2fit])
            data2fit_Gnknown_ADU= numpy.copy(dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol][map2fit])
            del map2fit
            map2fit= ~numpy.isnan(data2fit_Gnknown_ADU)
            data2fit_Gnknown_tint= numpy.copy(data2fit_Gnknown_tint[map2fit])
            data2fit_Gnknown_ADU= numpy.copy(data2fit_Gnknown_ADU[map2fit])
            del map2fit
            #
            map2fit= dataADU_avg_Gnunkwn_3DAr[:,thisRow,thisCol]<=(fit_maxADU)*numpy.nanmax(dataADU_avg_Gnunkwn_3DAr[:,thisRow,thisCol])
            data2fit_Gnunkwn_tint= numpy.copy(intTimes_ms_Gnunkwn[map2fit])
            data2fit_Gnunkwn_ADU= numpy.copy(dataADU_avg_Gnunkwn_3DAr[:,thisRow,thisCol][map2fit])
            del map2fit
            map2fit= ~numpy.isnan(data2fit_Gnunkwn_ADU)
            data2fit_Gnunkwn_tint= numpy.copy(data2fit_Gnunkwn_tint[map2fit])
            data2fit_Gnunkwn_ADU= numpy.copy(data2fit_Gnunkwn_ADU[map2fit])
            del map2fit
            #
            if ((len(data2fit_Gnknown_tint)>=fit_minNpoints)&(len(data2fit_Gnunkwn_tint)>=fit_minNpoints)):
                (fit_Gnknown_slope,fit_Gnknown_offset)= APy3_FITfuns.linear_fit(data2fit_Gnknown_tint,data2fit_Gnknown_ADU)
                fit_Gnknown_R2= APy3_FITfuns.linear_fit_R2(data2fit_Gnknown_tint,data2fit_Gnknown_ADU)
                APy3_GENfuns.printcol("{0} fit: slope={1}ADU/ms, offset={2}ms, R2={3}, Npoints={4}".format(label_Gnknown,fit_Gnknown_slope,fit_Gnknown_offset,fit_Gnknown_R2,len(data2fit_Gnknown_tint)),'green')
                #
                (fit_Gnunkwn_slope,fit_Gnunkwn_offset)= APy3_FITfuns.linear_fit(data2fit_Gnunkwn_tint,data2fit_Gnunkwn_ADU)
                fit_Gnunkwn_R2= APy3_FITfuns.linear_fit_R2(data2fit_Gnunkwn_tint,data2fit_Gnunkwn_ADU)
                APy3_GENfuns.printcol("{0} fit: slope={1}ADU/ms, offset={2}ms, R2={3}, Npoints={4}".format(label_Gnunkwn,fit_Gnunkwn_slope,fit_Gnunkwn_offset,fit_Gnunkwn_R2,len(data2fit_Gnunkwn_tint)),'green')
                #
                if ((fit_Gnknown_R2<fit_minR2)|(fit_Gnunkwn_R2<fit_minR2)): APy3_GENfuns.printcol("bad fit: R2=({0},{1}) < {2}".format(fit_Gnknown_R2,fit_Gnunkwn_R2, fit_minR2),'orange')
                elif ((fit_Gnknown_slope<fit_slope)|(fit_Gnunkwn_slope<fit_slope)): APy3_GENfuns.printcol("bad fit: slope=({0},{1}) < {2}".format(fit_Gnknown_slope,fit_Gnunkwn_slope, fit_slope),'orange')
                else:
                    if(numpy.isnan(e_per_ADU_Gnknown)): APy3_GENfuns.printcol("unable estimate e/ADU for {0}: no e/ADU data for {1}".format(label_Gnknown,label_Gnunkwn),'orange')
                    else:
                        e_per_ADU_Gnunkwn= e_per_ADU_multiGn_Gnknown[0,thisRow,thisCol] * (fit_Gnknown_slope / fit_Gnunkwn_slope)
                        APy3_GENfuns.printcol("{0}: {1}e/ADU".format(label_Gnknown,e_per_ADU_Gnknown),'green')
                        APy3_GENfuns.printcol("{0}: {1}e/ADU".format(label_Gnunkwn,e_per_ADU_Gnunkwn),'green')
            else: APy3_GENfuns.printcol("unable to fit: Npoints={0},{1} <{2}".format(len(data2fit_Gnknown_tint),len(data2fit_Gnunkwn_tint),fit_minNpoints),'orange')
            del data2fit_Gnknown_tint; del data2fit_Gnknown_ADU; del data2fit_Gnunkwn_tint; del data2fit_Gnunkwn_ADU
            #
            if pngFlag:
            ...
            #
                ...
                APy3_GENfuns.printcol("plotting pixel ({0},{1})".format(thisRow,thisCol),'green')
                #APy3_GENfuns.plot_1D_errbar(intTimes_ms_Gnknown,dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol],dataADU_std_Gnknown_3DAr[:,thisRow,thisCol], 'integration time [ms]','pixel output [ADU]', '{0}, pix({1},{2})'.format(label_Gnknown,thisRow,thisCol), False)
                #APy3_GENfuns.plot_1D_errbar(intTimes_ms_Gnunkwn,dataADU_avg_Gnunkwn_3DAr[:,thisRow,thisCol],dataADU_std_Gnunkwn_3DAr[:,thisRow,thisCol], 'integration time [ms]','pixel output [ADU]', '{0}, pix({1},{2})'.format(label_Gnunkwn,thisRow,thisCol), False)
                #
                #APy3_GENfuns.plot_1D_errbar(intTimes_ms_Gnknown,dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnknown,dataADU_std_Gnknown_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnknown, 'integration time [ms]','pixel output [e]', '{0}, pix({1},{2})'.format(label_Gnknown,thisRow,thisCol), False)
                #APy3_GENfuns.plot_1D_errbar(intTimes_ms_Gnunkwn,dataADU_avg_Gnunkwn_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnunkwn,dataADU_std_Gnunkwn_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnunkwn, 'integration time [ms]','pixel output [e]', '{0}, pix({1},{2})'.format(label_Gnunkwn,thisRow,thisCol), False)
                #
                if label_Gnknown in ['high gain','very-high gain']: 
    ...
    # not interactive
    APy3_GENfuns.printcol("non-interactive extraction",'blue')
    ...
    for thisRow in Rows2proc:
        for thisCol in Cols2proc:
            ...
            del map2fit
            #
            if verboseFlag: APy3_GENfuns.printcol("pix ({0},{1}):".format(thisRow,thisCol),'green')
            if ((len(data2fit_Gnknown_tint)>=fit_minNpoints)&(len(data2fit_Gnunkwn_tint)>=fit_minNpoints)):
                (fit_Gnknown_slope,fit_Gnknown_offset)= APy3_FITfuns.linear_fit(data2fit_Gnknown_tint,data2fit_Gnknown_ADU)
                fit_Gnknown_R2= APy3_FITfuns.linear_fit_R2(data2fit_Gnknown_tint,data2fit_Gnknown_ADU)
                map_Gnknown_R2[thisRow,thisCol]= fit_Gnknown_R2
                if verboseFlag: APy3_GENfuns.printcol("  {0} fit: slope={1}ADU/ms, offset={2}ms, R2={3}, Npoints={4}".format(label_Gnknown,fit_Gnknown_slope,fit_Gnknown_offset,fit_Gnknown_R2,len(data2fit_Gnknown_tint)),'green')
                #
                (fit_Gnunkwn_slope,fit_Gnunkwn_offset)= APy3_FITfuns.linear_fit(data2fit_Gnunkwn_tint,data2fit_Gnunkwn_ADU)
                fit_Gnunkwn_R2= APy3_FITfuns.linear_fit_R2(data2fit_Gnunkwn_tint,data2fit_Gnunkwn_ADU)
                map_Gnunkwn_R2[thisRow,thisCol]= fit_Gnunkwn_R2
                if verboseFlag: APy3_GENfuns.printcol("  {0} fit: slope={1}ADU/ms, offset={2}ms, R2={3}, Npoints={4}".format(label_Gnunkwn,fit_Gnunkwn_slope,fit_Gnunkwn_offset,fit_Gnunkwn_R2,len(data2fit_Gnunkwn_tint)),'green')
                #
                if ((fit_Gnknown_R2<fit_minR2)|(fit_Gnunkwn_R2<fit_minR2)): 
                    if verboseFlag: APy3_GENfuns.printcol("  bad fit: R2=({0},{1}) < {2}".format(fit_Gnknown_R2,fit_Gnunkwn_R2, fit_minR2),'orange')
                elif ((fit_Gnknown_slope<fit_slope)|(fit_Gnunkwn_slope<fit_slope)): 
                    if verboseFlag: APy3_GENfuns.printcol("bad fit: slope=({0},{1}) < {2}".format(fit_Gnknown_slope,fit_Gnunkwn_slope, fit_slope),'orange')
                else:
                    if(numpy.isnan(e_per_ADU_Gnknown)): 
                        if verboseFlag: APy3_GENfuns.printcol("  unable estimate e/ADU for {0}: no e/ADU data for {1}".format(label_Gnknown,label_Gnunkwn),'orange')
                    else:
                        e_per_ADU_Gnunkwn= e_per_ADU_multiGn_Gnknown[0,thisRow,thisCol] * (fit_Gnknown_slope / fit_Gnunkwn_slope)
                        if verboseFlag: 
                            APy3_GENfuns.printcol("  {0}: {1}e/ADU".format(label_Gnknown,e_per_ADU_Gnknown),'green')
                            APy3_GENfuns.printcol("  {0}: {1}e/ADU".format(label_Gnunkwn,e_per_ADU_Gnunkwn),'green')
                            e_per_ADU_multiGn_Gnunkwn[0,thisRow,thisCol]= e_per_ADU_Gnunkwn
            else: 
                if verboseFlag: APy3_GENfuns.printcol("  unable to fit: Npoints={0},{1} <{2}".format(len(data2fit_Gnknown_tint),len(data2fit_Gnunkwn_tint),fit_minNpoints),'orange')
            del data2fit_Gnknown_tint; del data2fit_Gnknown_ADU; del data2fit_Gnunkwn_tint; del data2fit_Gnunkwn_ADU
            #
            if (pngFlag & showRampsFlag):
                if verboseFlag: APy3_GENfuns.printcol("png-saving pixel ({0},{1})".format(thisRow,thisCol),'green')
                #APy3_GENfuns.png_1D_errbar(intTimes_ms_Gnknown,dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol],dataADU_std_Gnknown_3DAr[:,thisRow,thisCol], 'integration time [ms]','pixel output [ADU]', '{0}, pix({1},{2})'.format(label_Gnknown,thisRow,thisCol), False, pngFolder+'ramp_ADU_{0}_pix({1},{2})_1D.png'.format(label_Gnknown,thisRow,thisCol) )
                #APy3_GENfuns.png_1D_errbar(intTimes_ms_Gnunkwn,dataADU_avg_Gnunkwn_3DAr[:,thisRow,thisCol],dataADU_std_Gnunkwn_3DAr[:,thisRow,thisCol], 'integration time [ms]','pixel output [ADU]', '{0}, pix({1},{2})'.format(label_Gnunkwn,thisRow,thisCol), False, pngFolder+'ramp_ADU_{0}_pix({1},{2})_1D.png'.format(label_Gnunkwn,thisRow,thisCol) )
                #
                #APy3_GENfuns.png_1D_errbar(intTimes_ms_Gnknown,dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnknown,dataADU_std_Gnknown_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnknown, 'integration time [ms]','pixel output [e]', '{0}, pix({1},{2})'.format(label_Gnknown,thisRow,thisCol), False, pngFolder+'ramp_e_{0}_pix({1},{2})_1D.png'.format(label_Gnknown,thisRow,thisCol) )
                ...
                if label_Gnknown in ['high gain','very-high gain']: 
                    ...
                    APy3_GENfuns.png_errbar_1Dx3_samecanva([],[],[],[],
                                                           ...
                                                           pngFolder+'ramp_ADU_pix({0},{1})_1Dx2.png'.format(thisRow,thisCol))
                #
            elif (~pngFlag & showRampsFlag): 
                if verboseFlag: APy3_GENfuns.printcol("plotting pixel ({0},{1})".format(thisRow,thisCol),'green')
                #APy3_GENfuns.plot_1D_errbar(intTimes_ms_Gnknown,dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol],dataADU_std_Gnknown_3DAr[:,thisRow,thisCol], 'integration time [ms]','pixel output [ADU]', '{0}, pix({1},{2})'.format(label_Gnknown,thisRow,thisCol), False)
                #APy3_GENfuns.plot_1D_errbar(intTimes_ms_Gnunkwn,dataADU_avg_Gnunkwn_3DAr[:,thisRow,thisCol],dataADU_std_Gnunkwn_3DAr[:,thisRow,thisCol], 'integration time [ms]','pixel output [ADU]', '{0}, pix({1},{2})'.format(label_Gnunkwn,thisRow,thisCol), False)
                #
                #APy3_GENfuns.plot_1D_errbar(intTimes_ms_Gnknown,dataADU_avg_Gnknown_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnknown,dataADU_std_Gnknown_3DAr[:,thisRow,thisCol]*e_per_ADU_Gnknown, 'integration time [ms]','pixel output [e]', '{0}, pix({1},{2})'.format(label_Gnknown,thisRow,thisCol), False)
                ...
                if label_Gnknown in ['high gain','very-high gain']: 
                    ...
                    APy3_GENfuns.plot_errbar_1Dx3_samecanva([],[],[],[],
                                                            ...
                                                            'integration time [ms]','pixel output [ADU]','pix({0},{1})'.format(thisRow,thisCol), False)
                APy3_GENfuns.showIt()
        if (verboseFlag==False): APy3_GENfuns.printcol("row {0}: done".format(thisRow),'green')
    #---
    #%% show-plot map
    if pngFlag:
        APy3_GENfuns.png_2D_all(e_per_ADU_multiGn_Gnknown[0,:,:], False, 'col','row',"{0} [e/ADU]".format(label_Gnknown), True, pngFolder+'ADU2e_{0}_2D.png'.format(label_Gnknown,thisRow,thisCol) )
        APy3_GENfuns.png_2D_all(e_per_ADU_multiGn_Gnunkwn[0,:,:], False, 'col','row',"{0} [e/ADU]".format(label_Gnunkwn), True, pngFolder+'ADU2e_{0}_2D.png'.format(label_Gnknown,thisRow,thisCol) )
        APy3_GENfuns.png_2D_all(PedestalADU_multiGn_Gnknown[0,:,:], False, 'col','row',"{0} pedestal [ADU]".format(label_Gnknown), True, pngFolder+'ADU0_{0}_2D.png'.format(label_Gnknown,thisRow,thisCol) )
        APy3_GENfuns.png_2D_all(PedestalADU_multiGn_Gnunkwn[0,:,:], False, 'col','row',"{0} pedestal [ADU]".format(label_Gnunkwn), True, pngFolder+'ADU0_{0}_2D.png'.format(label_Gnknown,thisRow,thisCol) )
        APy3_GENfuns.printcol("png maps saved in {0}".format(pngFolder),'green')
    else:
        APy3_GENfuns.plot_2D_all(e_per_ADU_multiGn_Gnknown[0,:,:], False, 'col','row',"{0} [e/ADU]".format(label_Gnknown), True)
        APy3_GENfuns.plot_2D_all(e_per_ADU_multiGn_Gnunkwn[0,:,:], False, 'col','row',"{0} [e/ADU]".format(label_Gnunkwn), True)
        APy3_GENfuns.plot_2D_all(PedestalADU_multiGn_Gnknown[0,:,:], False, 'col','row',"{0} pedestal [ADU]".format(label_Gnknown), True)
        APy3_GENfuns.plot_2D_all(PedestalADU_multiGn_Gnunkwn[0,:,:], False, 'col','row',"{0} pedestal [ADU]".format(label_Gnunkwn), True)
        #
        APy3_GENfuns.plot_2D_all(map_Gnknown_R2, False, 'col','row',"{0} R^2".format(label_Gnknown), True)
        APy3_GENfuns.plot_2D_all(map_Gnunkwn_R2, False, 'col','row',"{0} R^2".format(label_Gnunkwn), True)
        APy3_GENfuns.showIt()
    #---
    #%% save
    if saveFlag:
        APy3_GENfuns.write_2xh5(saveFile, PedestalADU_multiGn_Gnunkwn,'/Pedestal_ADU/', e_per_ADU_multiGn_Gnunkwn,'/e_per_ADU/')
        APy3_GENfuns.printcol("multiGnCal_file for {0} saved as {1}".format(label_Gnunkwn,saveFile),'green')
#---
#%% that's all folks

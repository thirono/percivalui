...
class TestBoardValuesClass(unittest.TestCase):
    ...
    def TestFunctions(self):
        ...
        self.txrx.send_recv_message.assert_called_with(
            TxMessage(bytes("\x01\x97\x00\x00\x00\x00", encoding="latin-1"), num_response_msg=19, expect_eom=False))
        ...
        self.value.read_values()
        self.txrx.send_recv_message.assert_called_with(
            TxMessage(bytes("\x01\x95\x00\x00\x00\x00", encoding="latin-1"), num_response_msg=1, expect_eom=False))
        ...
        self.value.read_values()
        self.txrx.send_recv_message.assert_called_with(
            TxMessage(bytes("\x01\x96\x00\x00\x00\x00", encoding="latin-1"), num_response_msg=1, expect_eom=False))
        ...
        self.value.read_values()
        self.txrx.send_recv_message.assert_called_with(
            TxMessage(bytes("\x01\x98\x00\x00\x00\x00", encoding="latin-1"), num_response_msg=1, expect_eom=False))

...
class TestChannels(unittest.TestCase):
    ...
    def TestControlChannel(self):
        ...
        self.txrx.send_recv_message.assert_called_with(
            TxMessage(bytes("\x00\xF8\x20\x00\x00\x05", encoding="latin-1"), expect_eom=True))
        ...
        # Verify the txrx mock had send_rcv_message called
        self.txrx.send_recv_message.assert_called_with(
            TxMessage(bytes("\x00\xF8\x00\x00\x00\x05", encoding="latin-1"), expect_eom=True))
    ...
    def TestMonitoringChannel(self):
        ...
        self.assertEqual(calls[0], call(
            TxMessage(bytes("\x01\x51\x00\x00\x00\x00", encoding="latin-1"), expect_eom=False)))
        self.assertEqual(calls[1], call(
            TxMessage(bytes("\x00\xF8\x00\x80\x00\x12", encoding="latin-1"), expect_eom=True)))
        self.assertEqual(calls[2], call(
